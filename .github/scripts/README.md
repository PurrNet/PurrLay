...
Run **Deploy Purr Transport** from GitHub Actions. It runs the .NET session/recovery tests, Python deployment tests, and Linux IPv6 image smoke test before any Fly operations, then builds images, creates unmanaged Machines, hands off the balancer registry, and directs new rooms to the new relay generation. It does not update existing relay Machines or change their IP addresses. **Retire drained Purr Transport generations** runs hourly at minute 17 and can also be run manually. Both workflows share a concurrency group and never cancel an in-progress deployment or cleanup. `queue: max` also keeps scheduled cleanup from replacing a pending deployment in GitHub's default single-entry queue.
